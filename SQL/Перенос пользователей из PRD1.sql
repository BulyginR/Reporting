...
declare @intMaxLenLoginName int
declare @intMaxLenDBName int
declare @intMaxLenUserName int
set @intMaxLenLoginName=cast(@charMaxLenLoginName as int)
set @intMaxLenDBName=cast(@charMaxLenDBName as int)
set @intMaxLenUserName=cast(@charMaxLenUserName as int)


DECLARE vend_cursor CURSOR
    FOR 
select 
distinct LoginName = substring (LoginName  ,1 ,@intMaxLenLoginName )
         ,DBName = substring (DBName     ,1 , @intMaxLenDBName)
         ,UserName = substring (UserName   ,1 ,@intMaxLenUserName)
         ,UserOrAlias
  from
          #tb1_UA
          where
           substring (DBName   ,1 ,@intMaxLenDBName)='PRD1'
           and substring(substring (LoginName  ,1 , @intMaxLenUserName),1,14)='UNITED-EUROPE\'
           and UserOrAlias='User'
   order by
          1 ,2 ,3
declare @LoginName varchar(100)
declare @DBName varchar(100)
declare @UserName varchar(100)
declare @UserOrAlias varchar(100)

OPEN vend_cursor
FETCH NEXT FROM vend_cursor INTO @LoginName,@DBName,@UserName, @UserOrAlias
use ghost
